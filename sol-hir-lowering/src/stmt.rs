...
type SyntaxStmt<'tree> = sol_syntax::anon_unions::AskStmt_ExprStmt_IfStmt_LetStmt<'tree>;
...
impl HirLowering<'_, '_> {
    ...
    pub fn stmt(&mut self, stmt: SyntaxStmt, level: HirLevel) -> Stmt {
        use sol_syntax::anon_unions::AskStmt_ExprStmt_IfStmt_LetStmt::*;
    ...
    /// into a high-level statement.
    pub fn ask_stmt(&mut self, stmt: sol_syntax::AskStmt, level: HirLevel) -> Stmt {
    ...
    /// into a high-level statement.
    pub fn expr_stmt(&mut self, stmt: sol_syntax::ExprStmt, level: HirLevel) -> Stmt {
        let expr = stmt
            .child()
            .solve(self, |this, node| this.expr(node, level));
    ...
    /// into a high-level statement.
    pub fn if_stmt(&mut self, stmt: sol_syntax::IfStmt, level: HirLevel) -> Stmt {
        let scrutinee = stmt
            .condition()
            .solve(self, |this, node| this.expr(node, level));
    ...
    pub fn let_stmt(&mut self, stmt: sol_syntax::LetStmt, level: HirLevel) -> Stmt {
        let pattern = stmt.pattern().solve(self, |this, node| this.pattern(node));
        let expr = stmt
            .value()
            .solve(self, |this, node| this.expr(node, level));
    ...
    pub fn block(&mut self, block: sol_syntax::Block, level: HirLevel) -> Block {
