...
//!
//! This implementation uses tree-sitter, and resolves into `sol-hir`
...
use sol_hir::{
    ...
    source::{
        ...
        top_level::{
            ...
            DataDecl, InstanceDecl, Signature, TopLevel, TraitDecl, TypeDecl, UsingTopLevel,
        },
...
use sol_syntax::{anon_unions::ExplicitArguments_ImplicitArguments, Source, SourceFile};
...
#[rustfmt::skip]
type SyntaxVariant<'tree> = sol_syntax::anon_unions::Comma_FunctionConstructor_SignatureConstructor<'tree>;
...
impl<'db, 'tree> HirLowering<'db, 'tree> {
    ...
    pub fn define<'a>(&mut self, decl: SyntaxDecl<'a>) -> Option<Solver<'a, TopLevel>> {
        use sol_syntax::anon_unions::ClassDecl_Clause_Command_DataDecl_InstanceDecl_Signature_TraitDecl_TypeDecl_Using::*;
    ...
    /// concrete syntax tree [`sol_syntax::Command`].
    pub fn hir_command(&mut self, tree: sol_syntax::Command) -> TopLevel {
    ...
    /// concrete syntax tree [`sol_syntax::Using`].
    ///
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_using<'a>(&mut self, tree: sol_syntax::Using<'a>) -> Solver<'a, TopLevel> {
    ...
    /// concrete syntax tree [`sol_syntax::TypeDecl`].
    ///
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_type<'a>(&mut self, tree: sol_syntax::TypeDecl<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
    ...
    /// concrete syntax tree [`sol_syntax::ClassDecl`].
    ///
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_class<'a>(&mut self, tree: sol_syntax::ClassDecl<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
        let path = tree.name().solve(self, |this, node| this.path(node));
    ...
    /// concrete syntax tree [`sol_syntax::TraitDecl`].
    ///
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_instance<'a>(&mut self, tree: sol_syntax::InstanceDecl<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
    ...
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_trait<'a>(&mut self, tree: sol_syntax::TraitDecl<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
        let path = tree.name().solve(self, |this, path| this.path(path));
    ...
    /// concrete syntax tree [`sol_syntax::DataDecl`].
    ///
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_data<'a>(&mut self, tree: sol_syntax::DataDecl<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
        let path = tree.name().solve(self, |this, path| this.path(path));
    ...
    /// concrete syntax tree [`sol_syntax::Constructor`].
    ///
    ...
    pub fn hir_variant<'a>(&mut self, tree: SyntaxVariant<'a>) -> Option<Solver<'a, Constructor>> {
        use sol_syntax::anon_unions::Comma_FunctionConstructor_SignatureConstructor::*;
    ...
    /// syntax tree [`sol_syntax::Clause`].
    ///
    /// It will return a [`Solver`] for the [`Clause`], and it will solve the [`Clause`] in the
    /// [`hir_lower`] query.
    pub fn hir_clause<'a>(&mut self, tree: sol_syntax::Clause<'a>) -> Solver<'a, TopLevel> {
        let path = tree.name().solve(self, |this, path| this.path(path));
        let location = self.range(tree.range());
    ...
    /// It will return a [`Solver`] for the [`Signature`], and it will solve the [`Signature`] in
    /// the [`hir_lower`] query.
    pub fn hir_signature<'a>(&mut self, tree: sol_syntax::Signature<'a>) -> Solver<'a, TopLevel> {
        let range = self.range(tree.range());
        let path = tree.name().solve(self, |this, node| this.path(node));
        ...
        I: Iterator<Item = NodeResult<'a, ExtraOr<'a, ExplicitArguments_ImplicitArguments<'a>>>>,
    {
        arguments
            ...
            .flat_map(|parameter| {
                use sol_syntax::anon_unions::ExplicitArguments_ImplicitArguments::*;
    ...
    pub fn any_parameter(
        ...
        implicit: bool,
        rigid: bool,
        tree: sol_syntax::anon_unions::ForallParameter_Parameter,
    ) -> Parameter {
        use sol_syntax::anon_unions::ForallParameter_Parameter::*;
    ...
    /// It does takes rigid and implicit parameters, and it will return a [`Parameter`] with the
    /// given parameters.
    pub fn forall_parameter(&mut self, tree: sol_syntax::ForallParameter) -> Parameter {
    ...
    where
        I: Iterator<Item = NodeResult<'a, ExtraOr<'a, sol_syntax::DocString<'a>>>>,
    {
        attributes
            .flatten()
            .filter_map(|attr| {
    ...
    where
        I: Iterator<Item = NodeResult<'a, ExtraOr<'a, sol_syntax::Attribute<'a>>>>,
    {
        attributes
            .flatten()
    ...
    pub fn hir_visibility(&mut self, visibility: sol_syntax::Visibility) -> Spanned<Vis> {
    ...
    pub fn path(&mut self, path: sol_syntax::Path) -> HirPath {
        ...
        for segment in path.segments(&mut path.walk()) {
            segment.or_default_error(self, |this, segment: sol_syntax::Identifier| {
                let range = this.range(segment.range());
