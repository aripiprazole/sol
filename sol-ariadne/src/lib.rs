...
impl AriadneReport {
    ...
    pub fn write(self, output: &mut dyn std::io::Write) -> sol_eyre::Result<()> {
        write!(output, "{}", self.dump()?)?;
        Ok(())
    }

    pub fn dump(self) -> sol_eyre::Result<String> {
            ...
            ariadne::Report::<Span>::build(Error, file.path.clone(), 0)
                ...
                .with_labels(diagnostics.into_iter().filter_map(|d| {
                    ...
                    let kind = match d.error_kind() {
                        sol_diagnostic::ErrorKind::ParseError => "parse error",
    ...
    pub fn eprint(self) -> sol_eyre::Result<()> {
        println!("{}", self.dump()?);
        Ok(())
    }
