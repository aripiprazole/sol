...
#[derive(Debug)]
pub struct SolSemanticToken {
    pub start: usize,
    pub length: usize,
    pub token_type: SemanticTokenType,
}

pub fn semantic_highlight(text: &str) -> Vec<SolSemanticToken> {
    let mut highlighter = Highlighter::new();
    let mut config = HighlightConfiguration::new(
        /* language   = */ tree_sitter_sol::language(),
        /* highlights = */ tree_sitter_sol::HIGHLIGHTS_QUERY,
        /* injections = */ "",
        /* locals     = */ tree_sitter_sol::LOCALS_QUERY,
    )
    .unwrap();
        ...
        match event {
            ...
            Ok(HighlightEvent::Source { start, end }) => {
                ...
                if let Some(token_type) = lsp_token_type(name) {
                    semantic_tokens.push(SolSemanticToken {
                        length: end - start,
                        start,
                        token_type,
                    });
